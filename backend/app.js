  ...
  const DEEPSEEK_API_KEY = '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';
  ...
  app.post("/api/deepseek", authenticateToken, async (req, res) => {
      ...
      messages: [
        {
          role: "system",
          content:
            "Sen bir içerik denetleyicisisin. Sadece 'EVET' veya 'HAYIR' şeklinde cevap ver. Kullanıcıdan gelen içerik türkçe bir küfür içeriyorsa 'EVET' yaz, değilse 'HAYIR'.",
        },
        {
          role: "user",
          content: prompt,
    ...
    },
    {
      ...
      res.status(500).json({ error: "OpenRouter API çağrısında hata oluştu" });
    }
  });
